...
openai.api_key = "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"
...
def monitor_google_sheet(sheet_id, interval=30):
    ...
    while True:
        ...
        sheet_data = sheets_api.spreadsheets().values().get(
            spreadsheetId=sheet_id, range='Sheet1!A2:B'  # Update the range to include columns A to G
        ).execute()
        ...
        for row_idx, row in enumerate(rows[last_processed_row - 1:], start=last_processed_row):
            print(row)
            # exit()
            if len(row) >= 2 and row[0] and (len(row) == 2 or not row[2]):
                # short_prompt = f'Give me the Specifications of the product{pr}; Size, shape, texture specifically.''
                product = row[0]  # Update the index to 3 for the 4th column (Product)
                ...
                short_prompt = f'Give me short Specifications of {product} in the following format ;"Size: (in numeric value), Shape: (in one word) , Skin Color: (in one word and one color), Flesh Color: (in one word and one color), Texture: (less than two lines), Weight: (in Kg)"'
                ...
                print("short_comand = ", short_description)
                ...
                sheets_api.spreadsheets().values().update(
                    ...
                    range=f'Sheet1!A{row_idx + 1}:B{row_idx + 1}',  # Update the range to columns F and G
                    valueInputOption='RAW',
                    body={'values': [[short_description]]}
                ).execute()
...
if __name__ == '__main__':
    sheet_id = '1Dn3wokVXXtIM8AXhjSV14AiqnkByp3N2jSGKwSkhTm0'
